...
//       try {
//         const response = await axios.get('https://newsapi.org/v2/everything?q=Trending&from=2023-06-23&sortBy=publishedAt&apiKey=08ec0f24c26e46ccb06ea9b75210d020&language=en');
...
const formattedYesterday = yesterday.toISOString().slice(0, 10);
  ...
  const fetchNews = async () => {
    try {
      ...
      const response = await axios.get(`https://newsapi.org/v2/everything?q=Trending&from=${formattedYesterday}&sortBy=publishedAt&apiKey=${newsApiKey}&language=en`);
      setNews(response.data.articles);
    } catch (error) {
      console.log('An error occurred while fetching the news:', error);
    }
