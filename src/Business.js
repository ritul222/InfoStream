...
const formattedYesterday = yesterday.toISOString().slice(0, 10);
  ...
  const fetchNews = async () => {
    try {
      const newsApiKey =  process.env.REACT_APP_BUSINESS_API_KEY;  //process.env.BUSINESS_API_KEY;
      const response = await axios.get(`https://newsapi.org/v2/everything?q=Business&from=${formattedYesterday }&sortBy=publishedAt&apiKey=${newsApiKey}&language=en`);
